...
Scripts live at: `.claude/skills/ck/commands/` (project-level install; use `~/.claude/skills/ck/commands/` instead for a user-level install).
...
The hook at `.claude/skills/ck/hooks/session-start.mjs` (project-level install) must be
registered in `.claude/settings.json` to auto-load project context on session start.
Use `$CLAUDE_PROJECT_DIR` so the command resolves regardless of the harness's working
directory (for a user-level install, point at `~/.claude/skills/ck/hooks/session-start.mjs`
and register in `~/.claude/settings.json` instead):

```json
{
  "hooks": {
    "SessionStart": [
      { "hooks": [{ "type": "command", "command": "node \"$CLAUDE_PROJECT_DIR/.claude/skills/ck/hooks/session-start.mjs\"" }] }
    ]
  }
}
```
