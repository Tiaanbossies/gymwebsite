...
Add to your `.claude/settings.json` (project-level install; use `~/.claude/settings.json`
with the `~/.claude/skills/...` path instead for a user-level install). `$CLAUDE_PROJECT_DIR`
keeps the command resolving regardless of the harness's working directory:
