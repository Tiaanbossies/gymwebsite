...
async function serveStatic(req, res) {
  ...
  try {
    ...
    // Fall through to prerendered route check or SPA entry point.
  }

  // Serve prerendered static HTML generated by react-snap (e.g. dist/services/index.html).
  if (cleanPath) {
    const prerenderedPath = path.join(distDir, requestedPath, 'index.html');
    if (prerenderedPath.startsWith(distDir)) {
      try {
        const preStat = await fs.stat(prerenderedPath);
        if (preStat.isFile()) {
          res.writeHead(200, {
            'Content-Type': 'text/html; charset=utf-8',
          ...
          if (req.method === 'HEAD') { res.end(); return; }
          res.end(await fs.readFile(prerenderedPath));
          return;
        }
      } catch {
