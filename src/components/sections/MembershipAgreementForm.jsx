...
const openGymDefaultFromQuery = (value) => {
  ...
  if (value === 'm2m' || value === '6-month' || value === '12-month') return value;
  return 'm2m';
};
...
export default function MembershipAgreementForm() {
  ...
  const initialPlan = planFromQuery(queryPlan);
  const initialOpenGymPlan = openGymDefaultFromQuery(queryPlan);

  const [form, setForm] = useState({
    planType: initialPlan,
    openGymPlan: initialOpenGymPlan,
  ...
  const selectedPlan = useMemo(
    () => PLAN_OPTIONS.find((option) => option.value === form.planType) ?? PLAN_OPTIONS[0],
    [form.planType],
  );

  const selectedOpenGymPlan = useMemo(
    () =>
      OPEN_GYM_OPTIONS.find((option) => option.value === form.openGymPlan) ?? OPEN_GYM_OPTIONS[0],
    [form.openGymPlan],
  );
  ...
  const applicationCsv = useMemo(() => buildApplicationCsv(form, planSummary), [form, planSummary]);

  const shortWhatsappMessage = useMemo(
    () =>
      [
  ...
  const onHealthToggle = (item) => {
    setForm((current) => {
      ...
      return {
        ...current,
        healthFlags: exists
          ? current.healthFlags.filter((entry) => entry !== item)
  ...
  const validate = () => {
    ...
    if (!form.emergencyPhone.trim()) next.emergencyPhone = 'Please add an emergency contact number.';
    if (!form.consentAccuracy) next.consentAccuracy = 'Please confirm your information is correct.';
    if (!form.consentHealth) next.consentHealth = 'Please confirm your health disclosure acknowledgement.';
    if (!form.consentTerms) next.consentTerms = 'Please accept the membership terms summary.';
    if (!form.signatureName.trim()) next.signatureName = 'Please type your full name as your signature.';
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      setSendState({
        status: 'error',
        message: error.message || 'Something went wrong. Please try WhatsApp or email as a fallback.',
      });
    }
  };
  ...
  const downloadCopy = () => {
    ...
    anchor.href = url;
    anchor.download = `bossies-gym-membership-agreement-${slugify(form.fullName || 'member')}.csv`;
    ...
    return (
      ...
      >
        ...
        <div className="mt-6 flex flex-wrap justify-center gap-3">
          <a href={waLink(shortWhatsappMessage)} target="_blank" rel="noreferrer" className="btn-whatsapp">
    ...
    <form onSubmit={onSubmit} noValidate className="flex flex-col gap-8">
      <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 sm:p-6">
        <div className="flex items-start gap-3">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-brand-500/15 text-brand-300 ring-1 ring-brand-500/30">
            <FileText size={18} />
          </div>
          <div>
            ...
            <p className="mt-2 text-sm leading-relaxed text-ink-300">
              Start by choosing the plan you want this agreement to cover. If you're not 100% sure
              yet, pick the closest option and the gym can confirm the final fit with you.
            </p>
          </div>
        </div>
        ...
        <div className="mt-6 rounded-2xl border border-brand-500/25 bg-brand-500/5 p-4 text-sm leading-relaxed text-ink-200">
      ...
      <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 sm:p-6">
        <h3 className="font-display text-2xl tracking-headline text-white">Member details</h3>
      ...
      <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 sm:p-6">
        <h3 className="font-display text-2xl tracking-headline text-white">Emergency contact</h3>
        <div className="mt-6 grid gap-5 md:grid-cols-2">
      ...
      <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 sm:p-6">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="font-display text-2xl tracking-headline text-white">
              Health and training notes
            </h3>
            <p className="mt-2 text-sm leading-relaxed text-ink-300">
              This is here to help the team coach responsibly. If anything below applies, flag it so
              they can adjust training safely with you.
            </p>
          </div>
        </div>
      ...
      <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 sm:p-6">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-2 text-sm leading-relaxed text-ink-300">
              This confirms the basics only: your chosen plan, your health disclosure, and that the
              gym may contact you to finish onboarding. Final operational details still get confirmed
              directly with Bossie's team.
            </p>
          </div>
        </div>
      ...
      <div className="flex flex-col gap-4 rounded-2xl border border-brand-500/25 bg-brand-500/5 p-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-col gap-2">
          <p className="max-w-2xl text-sm leading-relaxed text-ink-200">
            No payment is taken on this page. When you send this through, the gym will confirm the
            final onboarding steps with you directly.
          </p>
...
function ChoiceCard({
  ...
  return (
    ...
    >
      ...
      <span className="mt-2 font-display text-2xl tracking-headline text-brand-200">{priceLine}</span>
...
function buildApplicationCsv(form, planSummary) {
  const rows = [
    [
      'submitted_at',
      'agreement_type',
      ...
      'health_disclosure_acknowledged',
      'plan_summary_acknowledged',
      'contact_consent',
      'signed_by',
      'signature_date',
    ],
