import { motion } from 'framer-motion';
import { Star, ExternalLink } from 'lucide-react';

import Container from '../ui/Container.jsx';
import SectionHeading from '../ui/SectionHeading.jsx';
import { stagger, fadeUp, site } from '../../lib/site.js';
import { googleReviews, totalReviewCount, overallRating } from '../../data/reviews.js';

/**
 * ReviewsSection — static Google Reviews curated from the live Business Profile.
 * Shows featured reviews in a 3-column grid with a summary badge and a
 * "See all on Google" link.
 *
 * To update: edit src/data/reviews.js — no API key or backend required.
 */
export default function ReviewsSection({ featuredOnly = true }) {
  const reviews = featuredOnly
    ? googleReviews.filter((r) => r.featured)
    : googleReviews;

  return (
    <section className="section-tight">
      <Container>
        <div className="flex flex-wrap items-end justify-between gap-6">
          <SectionHeading
            eyebrow="Google Reviews"
            title="What members say about Bossie's."
            description="Real reviews from our Google Business Profile — unedited."
          />

          <a
            href={site.socials.googleBusinessProfile}
            target="_blank"
            rel="noreferrer"
            aria-label={`${overallRating} stars — see all ${totalReviewCount} reviews on Google`}
            className="flex shrink-0 items-center gap-3 rounded-2xl border border-white/10 bg-ink-900 px-5 py-3 transition-colors hover:border-white/25"
          >
            <div className="flex items-center gap-0.5">
              {[...Array(5)].map((_, i) => (
                <Star key={i} size={14} className="fill-amber-400 text-amber-400" />
              ))}
            </div>
            <div className="border-l border-white/10 pl-3">
              <p className="text-sm font-semibold text-white">
                {overallRating.toFixed(1)} · {totalReviewCount} reviews
              </p>
              <p className="text-[11px] text-ink-400">See all on Google</p>
            </div>
            <ExternalLink size={14} className="text-ink-500" />
          </a>
        </div>

        <motion.ul
          variants={stagger}
          initial="hidden"
          whileInView="show"
          viewport={{ once: true, margin: '-60px' }}
          className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3"
        >
          {reviews.map((review, i) => (
            <ReviewCard key={i} review={review} />
          ))}
        </motion.ul>

        <div className="mt-8 text-center">
          <a
            href={site.socials.googleBusinessProfile}
            target="_blank"
            rel="noreferrer"
            className="btn-link inline-flex"
          >
            Read all {totalReviewCount} reviews on Google
            <ExternalLink size={13} strokeWidth={2.5} />
          </a>
        </div>
      </Container>
    </section>
  );
}

function ReviewCard({ review }) {
  const initials = review.name
    .split(' ')
    .slice(0, 2)
    .map((w) => w[0].toUpperCase())
    .join('');

  return (
    <motion.li
      variants={fadeUp}
      className="hover-lift flex flex-col gap-4 rounded-2xl border border-white/10 bg-ink-900 p-6 transition-colors hover:border-brand-500/30"
    >
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-brand-500/20 text-sm font-bold text-brand-300 ring-1 ring-brand-500/30">
            {initials}
          </div>
          <div>
            <p className="text-sm font-semibold text-white leading-tight">{review.name}</p>
            {review.isLocalGuide && (
              <p className="text-[10px] font-medium uppercase tracking-[0.15em] text-ink-500">
                Local Guide
              </p>
            )}
          </div>
        </div>
        <span className="shrink-0 text-[11px] text-ink-500">{review.date}</span>
      </div>

      <div className="flex items-center gap-0.5">
        {[...Array(review.rating)].map((_, i) => (
          <Star key={i} size={13} className="fill-amber-400 text-amber-400" />
        ))}
      </div>

      <p className="flex-1 text-sm leading-relaxed text-ink-300">{review.text}</p>

      <div className="flex items-center gap-2 border-t border-white/5 pt-3">
        <GoogleColourLogo />
        <span className="text-[11px] text-ink-500">Posted on Google</span>
      </div>
    </motion.li>
  );
}

function GoogleColourLogo() {
  return (
    <svg width="14" height="14" viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
      <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
      <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
      <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
    </svg>
  );
}
